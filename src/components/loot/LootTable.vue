...
/** Partition items by their group field. Returns array of item arrays. */
const groupedItems = computed(() => {
  const hasGroups = props.items.some(i => i.group !== undefined)
  if (!hasGroups) return [props.items]
  ...
  // Sort by group number and return arrays
  return [...map.entries()].sort((a, b) => a[0] - b[0]).map(([, items]) => items)
})

const isGrouped = computed(() => groupedItems.value.length > 1)
    ...
    <div :class="isGrouped ? 'md:columns-2 gap-3 space-y-3 md:space-y-0' : ''">
      ...
      >
        <div class="space-y-0.5">
          ...
          <div v-if="group.length === 0" class="text-[var(--text-secondary)] text-sm py-4 text-center">
            No items found.
          </div>
        </div>
      </div>
